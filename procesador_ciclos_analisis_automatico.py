...
concentracion =(1.62)*1e3 #[concentracion]= g/m^3 (1 g/l == 1e3 g/m^3) (Default = 10000 g/m^3)
...
if Analisis_de_Fourier==1:
    ...
    for i in range(len(fnames_m)):
        # Usar el índice normalizado para obtener el color
        color = cmap(norm(temp_m[i]))
        plt.plot(i+Ciclos_eje_H[i]/1000, Ciclos_eje_M_filt[i], '-', color=color)

    # plt.plot(1.0*len(fnames_m)+Ciclo_descancelacion_H/1000, Ciclo_descancelacion_M_filt, '-', color='k', label='Descancelación')
    plt.plot(Ciclos_eje_H[0]/1000, Ciclos_eje_M_filt[0], '.-', lw=2.2, color='tab:blue', label=f'{fnames_m[0].split("_")[-1][-7:-4]}  {temp_m[0]} °C')
    plt.plot(len(fnames_m)+Ciclos_eje_H[-1]/1000, Ciclos_eje_M_filt[-1], '.-', lw=2.2, color='tab:orange', label=f'{fnames_m[-1].split("_")[-1][-7:-4]}  {temp_m[-1]} °C')
    ...
    if templog:
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
        sm.set_array([])  # Esto es necesario para que la barra de colores muestre los valores correctos
        plt.colorbar(sm, label='Temperatura',ax=ax,orientation='horizontal', pad=0.02,fraction=0.1)  # Agrega una etiqueta adecuada
    ...
    plt.text(0.05, 0.9, f'{frec_nombre[0]/1000:>3.0f} kHz\n{round(np.mean(Campo_maximo)/1e3,2):>4.2f} kA/m',
             fontsize=12, bbox=dict(color='tab:orange', alpha=0.7), 
             transform=ax.transAxes, ha='left',va='top')
    plt.axhline(y=0, color='k', linestyle='-', lw=0.8,zorder=2)
    plt.axvline(x=0, color='k', linestyle='-', lw=0.8,zorder=-1)
    plt.axvline(x=1.0*len(fnames_m), color='k', linestyle='-', lw=0.8,zorder=-1)
    plt.grid(axis='y')
